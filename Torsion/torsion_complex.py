...
class TorsionComplexBuilder:
    ...
    def _d1_short(self, X, i: int) -> Dict[Hashable, Any]:
        ...
        if i == 0:
            return self._d_aux(X, [G.identity(), tet0.beta(0, 1, 2)], [0, 1], [-1, 1])
        if i == 3:
            return self._d_aux(X, [G.identity(), tet0.beta(1, 2, 0)], [1, 2], [-1, 1])
        if i == 6:
            return self._d_aux(X, [G.identity(), tet0.beta(3, 2, 1)], [2, 3], [-1, 1])
        if i == 9:
            return self._d_aux(X, [G.identity(), tet0.beta(2, 1, 3)], [3, 0], [-1, 1])
        if i == 1:
            return self._d_aux(X, [G.identity(), tet0.beta(0, 3, 1)], [0, 0], [-1, 1])
        if i == 4:
            return self._d_aux(X, [G.identity(), tet0.beta(1, 3, 2)], [1, 1], [-1, 1])
        if i == 7:
            return self._d_aux(X, [G.identity(), tet0.beta(3, 0, 2)], [2, 2], [-1, 1])
        if i == 10:
            return self._d_aux(X, [G.identity(), tet0.beta(2, 0, 1)], [3, 3], [-1, 1])
        if i == 2:
            return self._d_aux(X, [G.identity(), tet0.beta(0, 2, 3)], [1, 0], [-1, 1])
        if i == 5:
            return self._d_aux(X, [G.identity(), tet0.beta(1, 0, 3)], [2, 1], [-1, 1])
        if i == 8:
            return self._d_aux(X, [G.identity(), tet0.beta(3, 1, 0)], [3, 2], [-1, 1])
        if i == 11:
            return self._d_aux(X, [G.identity(), tet0.beta(2, 3, 0)], [0, 3], [-1, 1])
    ...
    def _d2_triangle(self, X, i: int) -> Dict[Hashable, Any]:
        ...
        if i == 0:
            return self._d_aux(X, [G.identity(), tet0.beta(0, 1, 2), tet0.beta(0, 1, 3)], [("short", 0), ("short", 2), ("short", 1)], [1, 1, 1])
        if i == 1:
            return self._d_aux(X, [G.identity(), tet0.beta(1, 2, 0), tet0.beta(1, 2, 3)], [("short", 3), ("short", 5), ("short", 4)], [1, 1, 1])
        if i == 2:
            return self._d_aux(X, [G.identity(), tet0.beta(3, 2, 1), tet0.beta(3, 2, 0)], [("short", 6), ("short", 8), ("short", 7)], [1, 1, 1])
        if i == 3:
            return self._d_aux(X, [G.identity(), tet0.beta(2, 1, 3), tet0.beta(2, 1, 0)], [("short", 9), ("short", 11), ("short", 10)], [1, 1, 1])
        if i == 4:
            return self._d_aux(X, [G.identity(), tet1.beta(0, 2, 3), tet1.beta(0, 2, 1)], [("short", 0), ("short", 4), ("short", 2)], [1, 1, 1])
        if i == 5:
            return self._d_aux(X, [G.identity(), tet1.beta(2, 3, 0), tet1.beta(2, 3, 1)], [("short", 3), ("short", 7), ("short", 5)], [1, 1, 1])
        if i == 6:
            return self._d_aux(X, [G.identity(), tet1.beta(3, 1, 0), tet1.beta(3, 1, 2)], [("short", 6), ("short", 10), ("short", 8)], [1, 1, 1])
        if i == 7:
            return self._d_aux(X, [G.identity(), tet1.beta(1, 0, 3), tet1.beta(1, 0, 2)], [("short", 9), ("short", 1), ("short", 11)], [1, 1, 1])
    ...
    def _d2_hexagon(self, X, i: int) -> Dict[Hashable, Any]:
        ...
        if i == 1:
            g_list = [
                tet0.beta(0, 3, 2),
                tet0.beta(2, 0, 3) * tet0.alpha(0, 2) * tet0.beta(0, 3, 2),
                G.identity(),
                tet0.beta(2, 0, 3) * tet0.alpha(0, 2) * tet0.beta(0, 3, 2),
                tet0.alpha(0, 3),
                tet0.beta(0, 3, 2),
        ...
        if i == 3:
            g_list = [
                G.identity(),
                tet0.beta(1, 0, 2) * tet0.alpha(0, 1),
                tet0.beta(0, 1, 2),
                tet0.beta(1, 0, 2) * tet0.alpha(0, 1),
                tet0.alpha(0, 2) * tet0.beta(0, 1, 2),
                G.identity(),
            ]
        ...
        if i == 0:
            g_list = [
                tet0.beta(1, 3, 2),
                tet0.beta(2, 1, 3) * tet0.alpha(1, 2) * tet0.beta(1, 3, 2),
                G.identity(),
                tet0.alpha(1, 2) * tet0.beta(1, 3, 2),
                tet0.beta(3, 1, 2) * tet0.alpha(1, 3),
                G.identity(),
            ]
        ...
        if i == 2:
            g_list = [
                tet0.alpha(1, 0) * tet0.beta(1, 3, 0),
                G.identity(),
                tet0.beta(0, 1, 3) * tet0.alpha(1, 0) * tet0.beta(1, 3, 0),
                tet0.beta(1, 3, 0),
                tet0.alpha(1, 3),
                tet0.beta(0, 1, 3) * tet0.alpha(1, 0) * tet0.beta(1, 3, 0),
            ]
    ...
    def _d3(self, X, i: int) -> Dict[Hashable, Any]:
        ...
        if i == 0:
            g_hexagon = [
                tet0.beta(1, 0, 3) * tet0.alpha(0, 1),
                tet0.beta(0, 1, 3),
                tet0.beta(1, 0, 3) * tet0.alpha(0, 1),
                G.identity(),
            ]
            g_triangle = [
                G.identity(),
                tet0.beta(1, 0, 2) * tet0.alpha(0, 1),
                tet0.beta(3, 0, 2) * tet0.alpha(0, 3) * tet0.beta(0, 1, 3),
                tet0.beta(2, 0, 1) * tet0.alpha(0, 2) * tet0.beta(0, 1, 2),
            ]
            return self._d_aux(
                X,
        ...
        if i == 1:
            g_hexagon = [
                tet1.beta(0, 1, 3),
                tet1.beta(0, 1, 2),
                tet1.beta(2, 0, 3) * tet1.alpha(0, 2) * tet1.beta(0, 1, 2),
                tet1.beta(0, 1, 2),
            ]
            g_triangle = [
                tet1.beta(0, 1, 2),
                tet1.beta(2, 0, 3) * tet1.alpha(0, 2) * tet1.beta(0, 1, 2),
                tet1.beta(3, 0, 1) * tet1.alpha(0, 3) * tet1.beta(0, 1, 3),
                tet1.alpha(0, 1),
            ]
            return self._d_aux(
                X,
